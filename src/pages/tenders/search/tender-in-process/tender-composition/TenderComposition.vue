  ...
  <div>
    <v-row>
      <v-toolbar>
        ...
        <v-row class="ml-0"> Состав закупки</v-row>
      </v-toolbar>
    </v-row>
          ...
          <v-card-text v-for="t in tenders" :key="t.id" @click="openTender(t)">
            <v-card v-show="!isCurrentTender(t)" outlined class="mb-3 pointer">
              <v-row class="d-flex px-3 justify-space-between pr-4">
                ...
                <v-card-actions>
                  <v-icon color="amber accent-3" @click="toggleFav(t)">{{
                    getStar(t.id)
                  }}</v-icon>
                </v-card-actions>
              </v-row>
              ...
              <v-card-text>
                <div>
                  <v-btn
                    ...
                    depressed
                    x-small
                  ><v-icon color="grey" size="16">mdi-map-marker</v-icon>{{ t.region_id }}</v-btn>
                  <v-btn
                    class="mr-1 mb-1"
                    color="pimary"
                    outlined
                    depressed
                    x-small
                  ><v-icon color="grey" size="16">mdi-file</v-icon>N{{
                  ...
                  <v-btn
                    ...
                    depressed
                    x-small
                  ><v-icon color="grey" size="16">mdi-bank</v-icon>{{ t.etp.short_name }}</v-btn>
                  <v-btn
                    class="mr-1 mb-1"
                    color="pimary"
                    outlined
                    depressed
                    x-small
                  ><v-icon color="grey" size="16">mdi-gavel</v-icon>{{ t.placing_way.type_name }}</v-btn>
                  <v-btn
                    class="mr-1 mb-1"
                    color="pimary"
                    outlined
                    depressed
                    x-small
                  ><v-icon color="grey" size="16">mdi-content-copy</v-icon>{{ t.id }}</v-btn>
                  <v-btn
                    class="mr-1 mb-1"
                    color="pimary"
                    outlined
                  ...
                  <v-btn
                    ...
                    depressed
                    x-small
                  >закончится {{ t.end_at | toNow() }}</v-btn>
                </div>
...
export default {
  ...
  data() {
    return {
      tab: null,
      tabs: [
        { text: 'Список позиций', id: 1 },
        { text: 'Список лотов', id: 2 },
        { text: 'Коды ОКПД2', id: 3 },
        { text: 'Заказчики', id: 4 }
      ],
      currentTender: null,
      favedTenders: []
